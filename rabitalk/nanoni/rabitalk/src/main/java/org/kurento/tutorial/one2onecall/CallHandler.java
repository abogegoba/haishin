...
public class CallHandler extends TextWebSocketHandler {
  ...
  private void userlist(WebSocketSession session)
      throws IOException {
   String userlist = "";
   for (int i = 0 ; i < registry.UName.size() ; i++){
	if(i>0) userlist = userlist + ",";
      userlist = userlist + registry.UName.get(i);
    }
    JsonObject response = new JsonObject();
    response.addProperty("id", "userlist");
    response.addProperty("userlist", userlist);
  ...
  private void register(WebSocketSession session, JsonObject jsonMessage) throws IOException {
   ...
   System.out.println("登録データ数:" + registry.UName.size());

    for (int i = 0 ; i < registry.UName.size() ; i++){
      String country = registry.UName.get(i);
...
/*
      for(int i=0; i<usercount; i++){
        if(users[i] == registry.getByName(stopperUser.getCallingTo())){
      ...
      usercount--;
      for(int i=0; i<usercount; i++){
        if(users[i] == registry.getByName(stoppedUser.getCallingTo())){
          for(int j=i;j<usercount;j++){
            users[j] = users[j+1];
          }
        }
